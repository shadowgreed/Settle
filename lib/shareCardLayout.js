...
const { SETTLE_WORDMARK_DATA_URI, SETTLE_WORDMARK_ASPECT } = require('./shareCardAssets');
...
// The real in-brand wordmark (src/assets/settle-wordmark.png) — was a 🎬
// emoji + hand-set "SETTLE" text, which renders with whatever color/style
// the font backing Satori's emoji glyph happens to use (a green clapperboard
// on this deploy), not an actual brand asset.
function Wordmark({ height = 36 }) {
  return h('img', {
    src: SETTLE_WORDMARK_DATA_URI,
    width: Math.round(height * SETTLE_WORDMARK_ASPECT),
    height,
  });
}
