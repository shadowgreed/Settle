 ...
 */
async function saveSubscription(uid, { subscription, topGenres, services, partnerUid }) {
  ...
  const profile = {
    ...
    // partnerUid attests "this user's linked partner". /api/ballot/notify uses
    // it to confirm a notification's sender is actually the recipient's partner,
    // so the endpoint can't be used to push arbitrary users. undefined (an old
    // client that doesn't send it) preserves the prior value; null clears it.
    partnerUid:
      partnerUid === undefined ? existing.partnerUid || null : partnerUid || null,
    lastSeenAt: Date.now(),
...
/**
 * Delete a user's ENTIRE push profile outright — every device's subscription,
 * targeting data, and watch-loop state — regardless of how many subs exist.
 * Used by account deletion (SEC-03 audit fix): removeSubscription above only
 * ever removes one device by endpoint, which left every OTHER device's
 * subscription (and the profile itself) live in Upstash after a user deleted
 * their account, since deleteUserData() only ever touched the Firestore doc.
 * Idempotent — safe to call on a uid with no profile at all.
 */
async function deleteProfile(uid) {
  if (!redis) return;
  await redis.del(profileKey(uid));
  await redis.srem(SET_KEY, uid);
}
...
/**
 * Schedule a "how was it?" watch-loop push for ~20h from now. Stored on the
 * user's push profile as `watchLoopPending: { titleName, sendAfter }`.
 * Only one pending watch-loop item per user at a time — a new one replaces
 * the old (the most recent watch is the most relevant thing to ask about).
 */
async function scheduleWatchLoop(uid, { titleName }) {
  if (!redis) return; // fail silently — push notification is optional
  const key = profileKey(uid);
  const existing = await redis.get(key);
  if (!existing) return; // no push profile = user isn't subscribed

  existing.watchLoopPending = {
    titleName: String(titleName || '').slice(0, 100),
    sendAfter:  Date.now() + 20 * 60 * 60 * 1000, // 20h from now
  };
  await redis.set(key, existing);
}

/**
 * Cron helper: clear the watchLoopPending field after the push is sent so
 * the same title isn't nudged twice.
 */
async function clearWatchLoop(uid) {
  if (!redis) return;
  const key = profileKey(uid);
  const existing = await redis.get(key);
  if (!existing) return;
  existing.watchLoopPending = null;
  await redis.set(key, existing);
}
